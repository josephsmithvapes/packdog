...
const REVIEWS = [
  { name: "Megan R.", dog: "Bruno, 3yr Malinois", stars: 5, text: "The hammock saved my leather seats on a 6-hour drive. Bruno destroyed everything before this. Absolute must-buy for road trip dogs.", product: "Car Hammock" },
...
const VIDEOS = [
  ...
  { src: `https://player.cloudinary.com/embed/?cloud_name=drlfyu2ee&public_id=vecteezy_an-adorable-tri-color-australian-shepherd-dog-happily-walks_75450584_kqyrg2`, label: "" },
  ...
  useEffect(() => {
    ...
    style.textContent = `
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .nav-link { font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 500; letter-spacing: 0.12em; text-transform: uppercase; color: #f5ede0; cursor: pointer; transition: color 0.2s; }
      .nav-link:hover { color: #e8962a; }
      .btn-primary { background: #c8721a; color: #fff; border: none; padding: 14px 32px; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; letter-spacing: 0.15em; text-transform: uppercase; cursor: pointer; transition: background 0.2s, transform 0.1s; }
      .btn-primary:hover { background: #e8862a; transform: translateY(-1px); }
      .btn-outline { background: transparent; color: #f5ede0; border: 1px solid #f5ede080; padding: 13px 28px; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 500; letter-spacing: 0.15em; text-transform: uppercase; cursor: pointer; transition: all 0.2s; }
      .btn-outline:hover { border-color: #f5ede0; background: #f5ede015; }
      .product-card { background: #fff; cursor: pointer; overflow: hidden; transition: transform 0.2s, box-shadow 0.2s; }
      .product-card:hover { transform: translateY(-4px); box-shadow: 0 12px 40px rgba(0,0,0,0.15); }
      .product-card img { width: 100%; height: 240px; object-fit: cover; display: block; transition: transform 0.4s; }
      .product-card:hover img { transform: scale(1.04); }
      .add-btn { width: 100%; padding: 12px; background: #122112; color: #f5ede0; border: none; font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 600; letter-spacing: 0.15em; text-transform: uppercase; cursor: pointer; transition: background 0.2s; }
      .add-btn:hover { background: #c8721a; }
      .filter-btn { background: transparent; border: 1px solid #2a2a2a40; padding: 8px 18px; font-family: 'DM Sans', sans-serif; font-size: 11px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #4a4a4a; cursor: pointer; transition: all 0.15s; }
      .filter-btn.active { background: #122112; color: #f5ede0; border-color: #122112; }
      .filter-btn:hover:not(.active) { border-color: #2a2a2a; color: #2a2a2a; }
      .review-card { background: #fff; padding: 24px; border-left: 3px solid #c8721a; }
      .cart-drawer { position: fixed; top: 0; right: 0; width: 380px; height: 100vh; background: #fff; z-index: 1000; display: flex; flex-direction: column; box-shadow: -8px 0 40px rgba(0,0,0,0.2); animation: slideIn 0.3s ease; }
      ...
      @media (max-width: 700px) {
        ...
        .video-mosaic { grid-template-columns: 1fr !important; }
      }
    `;
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#f5ede0", minHeight: "100vh" }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px" }}>
              {cart.length === 0 ? (
                ...
                <div key={item.id} style={{ display: "flex", gap: 12, marginBottom: 16, paddingBottom: 16, borderBottom: "1px solid #f0f0f0" }}>
                  <img src={item.image} alt={item.name} style={{ width: 70, height: 70, objectFit: "cover" }} />
                  <div style={{ flex: 1 }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
            ...
            <div className="nav-links" style={{ display: "flex", gap: 24, alignItems: "center" }}>
              {["Shop", "About", "Reviews"].map(l => <span key={l} className="nav-link">{l}</span>)}
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            ...
            <button onClick={() => setCartOpen(true)} style={{ background: "none", border: "1px solid #2a4a2a", color: "#f5ede0", padding: "8px 16px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontFamily: "'DM Sans'", fontSize: 12, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase" }}>
        ...
        <div style={{ display: "flex", animation: "ticker 20s linear infinite", whiteSpace: "nowrap" }}>
          {[...Array(4)].map((_, ri) => (
            <span key={ri} style={{ display: "flex", gap: 40, paddingRight: 40 }}>
              {["🐕 Adventure dog gear", "🚗 Free shipping over $50", "⚡ Ships in 5-8 days", "🏔 Trail and road tested", "✓ 30-day guarantee", "🐾 10% to Best Friends Animal Society"].map((t, i) => (
      ...
      <div style={{ position: "relative", height: "92vh", minHeight: 560, overflow: "hidden", display: "flex", alignItems: "center" }}>
        <img src="https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=1400&q=80" alt="Adventure dog" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
        ...
        <div className="hero-inner" style={{ position: "relative", zIndex: 2, maxWidth: 1200, margin: "0 auto", padding: "0 24px", animation: "fadeUp 0.8s ease both" }}>
          ...
          <h1 className="hero-headline" style={{ fontFamily: "'Bebas Neue'", fontSize: 96, color: "#f5ede0", lineHeight: 0.95, letterSpacing: 3, maxWidth: 700, marginBottom: 24 }}>
            ...
            <span style={{ color: "#c8721a" }}>GO PLACES</span>
          </h1>
      ...
      <div style={{ background: "#1a3a1c", padding: "20px 40px" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
          {[
            ...
            ["🚗", "Ships in 5-8 Days", "From US warehouse"],
          ].map(([icon, title, sub]) => (
            <div key={title} style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 22 }}>{icon}</span>
              <div>
                <div style={{ fontFamily: "'DM Sans'", fontSize: 12, fontWeight: 700, color: "#f5ede0" }}>{title}</div>
      ...
      <div id="shop" style={{ maxWidth: 1200, margin: "0 auto", padding: "72px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 40, flexWrap: "wrap", gap: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {categories.map(c => (
              <button key={c} className={`filter-btn ${filter === c ? "active" : ""}`} onClick={() => setFilter(c)} style={{ textTransform: "capitalize" }}>{c}</button>
            ))}
          </div>
        </div>
        <div className="product-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
          {loading ? (
            <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "60px 0", fontFamily: "'Lora'", color: "#8b6a4a", fontSize: 15, fontStyle: "italic" }}>Loading gear...</div>
              ...
              <div style={{ position: "relative", overflow: "hidden" }}>
                ...
                {product.badge && (
                  <div style={{ position: "absolute", top: 12, left: 12 }}>
                    <span style={{ background: "#c8721a", color: "#fff", fontFamily: "'DM Sans'", fontSize: 9, fontWeight: 700, letterSpacing: 2, padding: "4px 8px", textTransform: "uppercase" }}>{product.badge}</span>
                  </div>
                )}
                <div style={{ position: "absolute", top: 12, right: 12 }}>
                  <span style={{ background: "#f5ede0", color: "#4a4a4a", fontFamily: "'DM Sans'", fontSize: 9, fontWeight: 600, letterSpacing: 1.5, padding: "4px 8px", textTransform: "uppercase" }}>{product.category}</span>
                </div>
                {added === product.id && (
      ...
      <div style={{ background: "#122112", padding: "60px 24px" }}>
        <div className="bundle-grid" style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "center" }}>
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32 }}>
              <span style={{ fontFamily: "'Bebas Neue'", fontSize: 48, color: "#c8721a", letterSpacing: 2 }}>$74</span>
              <div>
                <div style={{ fontFamily: "'DM Sans'", fontSize: 12, color: "#c8d4b070", textDecoration: "line-through" }}>$84 separate</div>
                <div style={{ fontFamily: "'DM Sans'", fontSize: 12, fontWeight: 700, color: "#6abf6a", letterSpacing: 1 }}>SAVE $10</div>
              </div>
            </div>
      ...
      <div style={{ background: "#f5ede0", padding: "80px 24px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontFamily: "'Lora'", fontSize: 16, color: "#4a4a4a", lineHeight: 1.9, marginBottom: 20 }}>
            PackDog started because adventure dogs deserve gear that keeps up with them. Not generic pet store stuff — real, tested, trail-ready gear for dogs that ride shotgun on road trips, leap into rivers, and somehow always find the muddiest path.
          </p>
          <p style={{ fontFamily: "'Lora'", fontSize: 15, color: "#6b6b6b", lineHeight: 1.9 }}>
            Whether you are driving cross-country, hitting a weekend trail, or just living life with a dog who never sits still — we have got the gear.
          </p>
            ...
            {REVIEWS.map((r, i) => (
              <div key={i} className="review-card">
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                  ...
                  <span style={{ fontFamily: "'DM Sans'", fontSize: 9, fontWeight: 700, letterSpacing: 1.5, color: "#c8721a", background: "#f5ede0", padding: "4px 8px", textTransform: "uppercase" }}>{r.product}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ background: "#091509", padding: "60px 24px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div className="video-mosaic" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gridTemplateRows: "auto", gap: 8 }}>
            <div style={{ gridRow: "1 / 3", position: "relative", overflow: "hidden", border: "1px solid #2a4a2a", background: "#0a1a0a" }}>
              <video src={VIDEOS[0].src} autoPlay muted loop playsInline style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", minHeight: 320 }} />
            ...
            <div style={{ position: "relative", overflow: "hidden", border: "1px solid #2a4a2a", background: "#0a1a0a" }}>
              <video src={VIDEOS[1].src} autoPlay muted loop playsInline style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", minHeight: 156 }} />
            </div>
            <div style={{ position: "relative", overflow: "hidden", border: "1px solid #2a4a2a", background: "#0a1a0a" }}>
              <video src={VIDEOS[2].src} autoPlay muted loop playsInline style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", minHeight: 156 }} />
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32, flexWrap: "wrap", gap: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }}>
              {[["Shop", ["All Products", "Car Gear", "Trail Gear", "Bundles"]], ["Company", ["About", "Contact", "Returns", "Donate"]]].map(([title, links]) => (
                <div key={title}>
                  ...
                  {links.map(l => <div key={l} style={{ fontFamily: "'DM Sans'", fontSize: 12, color: "#4a6a4a", marginBottom: 8, cursor: "pointer" }}>{l}</div>)}
                </div>
              ))}
            </div>
          </div>
